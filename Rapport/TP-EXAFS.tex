...
We will first present the theoretical aspects of the EXAFS and the principles of the synchrotron used. Then, we will show how to prepare the line for measurements, how to make samples and some exploitations of the results given by the EXAFS method.
...
Figure\ref{Incident}, shows the beginning of the beamline we used (BM23), the tunnel on the right is the exit of storage ring and the first component focus X-ray beam in the beamline.
...
The first interesting component is the monochromator on figure \ref{monochromateur}. When the breaking radiation is created, it got a wide band of energy, the monochromator uses Bragg reflection to keep only X-rays of energy selected. Bragg's law is the equation \ref{Bragg}. This equation shows that the dispersion in wavelength $\lambda$ cause a dispersion in reflection angle. The monochromator uses this to shut down X-rays that don't have the right energy.
...
Now we have got the correct X-ray, the measure is simple, we just need to put our sample in the beamline (figure \ref{ligne_mesure}) and measure X-ray intensity before and after passing through the sample. The intensity difference gives us the absorption induced by the crossing of our sample.
